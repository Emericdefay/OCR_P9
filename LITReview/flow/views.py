...
def flow(request):
    ...
    if request.user.is_authenticated:
        ...
        t_from_followed_user = tickets.filter(
        ...
        r_from_followed_user = reviews.filter(
            user__id__in=users.filter(
                user=request.user).values_list("followed_user_id")
                                             )
        # 3. Get reviews from un-followed_user-s that review the user's ticket
        r_from_unfollowed_reviewed = reviews.filter(
            ticket__id__in=tickets.filter(
                user=request.user).values_list("id")
                                                   )
        # Union :
        union_reviews = (
            r_from_user | r_from_followed_user | r_from_unfollowed_reviewed
                        )

        # Sort all posts by -time_created:
        flow_posts = list(sorted(
                        chain(union_tickets, union_reviews),
                        key=attrgetter("time_created"),
                        reverse=True))
        return render(
                    request,
                    "flow/flow.html",
                    context={"data": flow_posts, "range_5": range(5)})
    else:
