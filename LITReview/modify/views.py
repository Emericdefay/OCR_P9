    ...
    try:
        if request.method == "GET":
            ...
            if content == "ticket":
                ...
                if form.is_valid():
                    Ticket.objects.filter(Q(id=id_modify)&Q(user=request.user)).update(title= form.cleaned_data["title"])
                    Ticket.objects.filter(Q(id=id_modify)&Q(user=request.user)).update(description= form.cleaned_data["description"])
                    pass    
                return redirect("/posts/")
            elif content == "review":
                form = CreateReview(request.POST)
                if form.is_valid():
                    Review.objects.filter(Q(id=id_modify)&Q(user=request.user)).update(headline= form.cleaned_data["headline"])
                    Review.objects.filter(Q(id=id_modify)&Q(user=request.user)).update(rating= form.cleaned_data["rating"])
                    Review.objects.filter(Q(id=id_modify)&Q(user=request.user)).update(body= form.cleaned_data["body"])
                    pass
                return redirect("/posts/")
